...
def compute_bounding_box_from_mask(mask):
    ...
    Returns: (min_x, min_y, max_x, max_y, width, height, center_x, center_y)
    """
    if np.sum(mask) == 0:
...
def compute_apparent_size_metrics(mask, bbox_data):
    """
    Compute various apparent size metrics from mask and bounding box.
    """
    pixel_count = int(np.sum(mask))
    
    if bbox_data[0] is None:  # No valid bounding box
        return {
            ...
            "pixel_density": 0.0,
            "equivalent_radius_pixel": 0.0,
            "equivalent_radius_bbox": 0.0,
            "bbox_center": [0, 0]
        }
    
    min_x, min_y, max_x, max_y, width, height, center_x, center_y = bbox_data
    ...
    # Equivalent radius assuming circular object
    equivalent_radius_pixel = np.sqrt(pixel_count / np.pi) if pixel_count > 0 else 0.0
    equivalent_radius_bbox = np.sqrt(bbox_area / np.pi) if bbox_area > 0 else 0.0
    ...
    return {
        ...
        "pixel_density": float(pixel_density),  # What fraction of bbox is actual object
        "equivalent_radius_pixel": float(equivalent_radius_pixel),
        "equivalent_radius_bbox": float(equivalent_radius_bbox),
...
# Enhanced mask analysis with bounding boxes
...
if "category_id_segmaps" in data:
    ...
    # Calculate enhanced mask data for each cube
    for i, cube_idx in enumerate([1, 2]):
        ...
        # Compute all apparent size metrics
        size_metrics = compute_apparent_size_metrics(mask, bbox_data)
        
        # Legacy compatibility - keep original fields
        cube_mask_data[cube_key] = {
            "mask_pixel_count": size_metrics["pixel_count"],
            "mask_coverage_percent": float(size_metrics["pixel_count"] / (image_width * image_height) * 100),
            # New bounding box fields
            "bbox_width": size_metrics["bbox_width"],
            "bbox_height": size_metrics["bbox_height"], 
            "bbox_area": size_metrics["bbox_area"],
            "bbox_aspect_ratio": size_metrics["bbox_aspect_ratio"],
            "bbox_center": size_metrics["bbox_center"],
            "bbox_coords": size_metrics["bbox_coords"],
            # New analytical features
            "pixel_density": size_metrics["pixel_density"],
            "equivalent_radius_pixel": size_metrics["equivalent_radius_pixel"],
            "equivalent_radius_bbox": size_metrics["equivalent_radius_bbox"],
            # Additional coverage metrics
            "bbox_coverage_percent": float(size_metrics["bbox_area"] / (image_width * image_height) * 100)
        }
        
        print(f"Cube {i+1}: Pixels={size_metrics['pixel_count']}, "
              f"BBox={size_metrics['bbox_width']}x{size_metrics['bbox_height']}, "
              f"Density={size_metrics['pixel_density']:.3f}, "
              f"AspectRatio={size_metrics['bbox_aspect_ratio']:.3f}")
